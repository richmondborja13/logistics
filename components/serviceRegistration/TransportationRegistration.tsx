/*
============================================================
TransportationRegistration Component (Front-end Guide & Back-end Follow Through)
============================================================

Front-end Guide:
- This file implements the registration form for transportation providers.
- Uses react-hook-form and zod for form handling and validation.
- To update form fields or validation, edit the transportationSchema and form JSX.
- To change layout or content, edit the JSX in the TransportationRegistration component.
...
const transportationSchema = z.object({
    ...
    fleetSize: z.number().min(1, { message: "Fleet size must be at least 1." }),
    coverageArea: z.string().min(2, { message: "Coverage area is required." }),
...
const TransportationRegistration: React.FC = () => {
    const router = useRouter();
    const { register, handleSubmit, formState: { errors } } = useForm<TransportationFormData>({
    ...
    const onSubmit = (data: TransportationFormData) => {
        console.log('Transportation Register:', data);
    };

    return (
        <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
            <h2 className="mb-2 text-2xl font-bold text-center">Register as Transportation Provider</h2>
            <p className="mb-6 text-center text-gray-500 text-sm">
                Provide details about your fleet, coverage area, and certifications.
            </p>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Fleet Size</label>
                            <input type="number" className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., 25 vehicles" {...register('fleetSize', { valueAsNumber: true })} />
                            {errors.fleetSize && <span className="text-red-500 text-sm">{errors.fleetSize.message}</span>}
                        </div>
                        
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Coverage Area</label>
                            <input className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., Northeast US, California" {...register('coverageArea')} />
                            {errors.coverageArea && <span className="text-red-500 text-sm">{errors.coverageArea.message}</span>}
                        </div>
                        
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Certifications (Optional)</label>
                            <input className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., DOT, CDL, ISO 9001" {...register('certifications')} />
                        </div>
                    </div>

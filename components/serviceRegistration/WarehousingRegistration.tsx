/*
============================================================
WarehousingRegistration Component (Front-end Guide & Back-end Follow Through)
============================================================

Front-end Guide:
- This file implements the registration form for warehousing providers.
- Uses react-hook-form and zod for form handling and validation.
- To update form fields or validation, edit the warehousingSchema and form JSX.
- To change layout or content, edit the JSX in the WarehousingRegistration component.
...
const warehousingSchema = z.object({
    companyName: z.string().min(2, { message: "Company name must be at least 2 characters." }),
    storageCapacity: z.string().min(1, { message: "Storage capacity is required." }),
    facilityType: z.string().min(2, { message: "Facility type is required." }),
...
type WarehousingFormData = z.infer<typeof warehousingSchema>;

const WarehousingRegistration: React.FC = () => {
    const router = useRouter();
    const { register, handleSubmit, formState: { errors } } = useForm<WarehousingFormData>({
        resolver: zodResolver(warehousingSchema),
        defaultValues: {}
    });
    const [modal, setModal] = React.useState<null | 'consent' | 'privacy'>(null);

    const onSubmit = (data: WarehousingFormData) => {
        console.log('Warehousing Register:', data);
    };

    return (
        <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
            <h2 className="mb-2 text-2xl font-bold text-center">Register as Warehousing Provider</h2>
            <p className="mb-6 text-center text-gray-500 text-sm">
                Provide details about your storage capacity, facility type, and security features.
            </p>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Storage Capacity</label>
                            <input className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., 50,000 sq ft, 1000 pallets" {...register('storageCapacity')} />
                            {errors.storageCapacity && <span className="text-red-500 text-sm">{errors.storageCapacity.message}</span>}
                        </div>
                        
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Facility Type</label>
                            <input className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., Cold storage, General warehouse" {...register('facilityType')} />
                            {errors.facilityType && <span className="text-red-500 text-sm">{errors.facilityType.message}</span>}
                        </div>
                        
                        <div>
                            <label className="block mb-1 text-sm text-gray-600">Security Features (Optional)</label>
                            <input className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., 24/7 surveillance, access control" {...register('securityFeatures')} />
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <input className="w-full px-3 py-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="e.g., ISO 9001, HACCP, FDA" {...register('certifications')} />
                        </div>
...
export default WarehousingRegistration; 